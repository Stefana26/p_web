...
public class GenreController : AuthorizedController
{
    private readonly IGenreService _genreService;
    public GenreController(IUserService userService, IGenreService genreService) : base(userService)
    {
    ...
    public async Task<ActionResult<RequestResponse>> Update([FromBody] GenreUpdateDTO genre) // The FromBody attribute indicates that the parameter is deserialized from the JSON body.
    {
        ...
        return currentUser.Result != null ?
            this.FromServiceResponse(await _genreService.UpdateGenre(genre, currentUser.Result)) :
            this.ErrorMessageResult(currentUser.Error);
    }
    ...
    public async Task<ActionResult<RequestResponse>> Delete([FromRoute] Guid id) // The FromRoute attribute will bind the id from the route to this parameter.
    {
        ...
        return currentUser.Result != null ?
            this.FromServiceResponse(await _genreService.DeleteGenre(id)) :
            this.ErrorMessageResult(currentUser.Error);
    }
}
